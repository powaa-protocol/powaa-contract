...
import { CurveLPVaultMigrator__factory } from "../../typechain";
import {
  assertEnvHOF,
  ETH_NETWORK,
  getAddress,
...
export async function main(): Promise<void> {
  ...
  console.log(">> 1. Deploying Curve LP TokenVault Migrator Contract...");
  const TokenVaultMigrator = await new CurveLPVaultMigrator__factory(
    deployer
  ).deploy(
  ...
  console.log(
    `>> ✅  Done Deploying Curve LP TokenVault Migrator Contract with address ${TokenVaultMigrator.address}`
  );
  ...
  console.log(
    ">> 2. Deploying Curve LP TokenVault Reserve Migrator Contract..."
  );
  const TokenVaultReserveMigrator = await new CurveLPVaultMigrator__factory(
    deployer
  ).deploy(
    tokenVaultMigratorParams.treasuryAccount,
    controllerAddress,
  ...
  console.log(
    `>> ✅  Done Deploying Curve LP TokenVault Reserve Migrator Contract with address ${TokenVaultReserveMigrator.address}`
  );
