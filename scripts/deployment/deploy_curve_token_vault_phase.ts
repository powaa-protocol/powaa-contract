...
export async function main(): Promise<void> {
  ...
  const tokenVaultTotalIncentives = parseEther("260000000000"); // 260,000,000,000.00
  ...
  const stakingTokenSymbol = "LOOKS";
  ...
  const mapTokenVaultRouterParams = {
    curveLPRouter: getAddress("", LAYER_NETWORK, env),
    coinCount: 3,
  };
  ...
  // 6) TokenVault Notify reward amount based on rewardsDuration * rewardPerSec
  console.log(
    `>> 6. Notify reward amount ${tokenVaultRewardOptionParams.rewardAmount.toString()}`
  );
