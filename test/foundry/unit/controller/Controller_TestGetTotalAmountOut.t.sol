...
contract Controller_TestGetTotalAmountOut is ControllerBaseTest {
  ...
  function test_WhenSomeVaultDeployed() external {
    ...
    stakingToken3.mint(vaults[2], 15 ether);
    MockTokenVault(payable(address(vaults[2]))).mockSetEthConversionRate(
      1 ether
    );

    uint256 totalAmount = controller.getTotalAmountOut();
    assertEq(45 ether, totalAmount);
  }
}
